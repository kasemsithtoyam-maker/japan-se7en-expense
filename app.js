...
function refCol() {
  return collection(db, `profiles/${selectedEmail}/expenses`);
}

function refBalancesDoc() {
  return doc(db, `profiles/${selectedEmail}/settings/balances`);
}
...
btnExport.onclick = async () => {
  ...
  const blob = new Blob(
    [JSON.stringify({ email: selectedEmail, exportedAt: Date.now(), data }, null, 2)],
  ...
  a.href = url;
  a.download = `expenses_${selectedEmail}_${new Date().toISOString().slice(0, 10)}.json`;
  a.click();
...
async function loadBalances() {
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      if (d.startDate) {
        tripStartDate.valueAsDate = new Date(d.startDate);
      }
    } else {
      balYouTrip.value = "";
...
btnSaveBalances.onclick = async () => {
  ...
  try {
    ...
    console.error(e);
    setSyncBadge('error');
    alert('ไม่สามารถบันทึกยอดตั้งต้น: ' + e.message);
  }
};
  ...
  const html = `
    <table>
      <thead>
        <tr>
          <th>บัตร</th>
          <th>ยอดตั้งต้น (THB)</th>
          <th>ใช้ไปวันนี้ (THB)</th>
          <th>ควรคงเหลือ (THB)</th>
        </tr>
      </thead>
      <tbody>
        ${rows.map(r => `<tr>
